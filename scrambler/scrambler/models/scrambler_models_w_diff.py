...
def sample_position_mask_st(mask_logits, temperature=None) :
    n_examples = K.shape(mask_logits)[0]
    input_size_x = K.shape(mask_logits)[1]
    input_size_y = K.shape(mask_logits)[2]

    flat_mask = K.reshape(mask_logits, (n_examples, input_size_x * input_size_y))
    sampled_mask = st_sampled_softmax(flat_mask)

    return K.reshape(sampled_mask, (n_examples, input_size_x, input_size_y, 1))

def sample_position_mask_gumbel(mask_logits, temperature=0.5) :
    n_examples = K.shape(mask_logits)[0]
    input_size_x = K.shape(mask_logits)[1]
    input_size_y = K.shape(mask_logits)[2]

    flat_mask = K.reshape(mask_logits, (n_examples, input_size_x * input_size_y))
    sampled_mask = gumbel_softmax(flat_mask, temperature=temperature)
...
def build_sampler(batch_size, input_size_x, input_size_y, n_classes=1, n_samples=1, subset_size=None, sample_mode='st', n_channels=4, gumbel_temp=0.5, model_prefix='') :
    ...
    if subset_size is not None :
        ...
        subset_sample_func = None
        if sample_mode == 'st' :
            subset_sample_func = sample_position_mask_st
        elif sample_mode == 'gumbel' :
            subset_sample_func = sample_position_mask_gumbel
        
        subset_scale_conv = Conv2D(1, (1, 1), strides=(1, 1), padding='same', activation='linear', kernel_initializer='ones', use_bias=False, name='scrambler_subset_scale_conv')
        
        subset_upsampling_layer = Lambda(lambda x: K.tile(x, [subset_size, 1, 1, 1]), name=model_prefix + 'subset_upsampling_layer')
        subset_sampling_layer = Lambda(lambda x: subset_sample_func(x, temperature=gumbel_temp), name=model_prefix + 'subset_pwm_sampler')
        subset_permute_layer = Lambda(lambda x: K.max(K.permute_dimensions(K.reshape(x, (subset_size, batch_size, input_size_x, input_size_y, 1)), (1, 0, 2, 3, 4)), axis=1), name=model_prefix + 'subset_permute_layer')
        
        subset_gate_layer = Lambda(lambda x: x[0] * K.tile(x[1], (1, 1, 1, n_channels)), name=model_prefix + 'subset_gate_layer')
    
    def _sampler_func(class_input, raw_logits, raw_scores=None) :
        ...
        #Optionally Apply Subset Mask
        if subset_size is not None and raw_scores is not None :
            scores_upsampled = subset_upsampling_layer(subset_scale_conv(raw_scores))
            sampled_subset = subset_sampling_layer(scores_upsampled)
            sampled_subset = subset_permute_layer(sampled_subset)
            
            raw_logits = subset_gate_layer([raw_logits, sampled_subset])
...
def load_scrambler_network(input_size_x, input_size_y, scrambler_mode='inclusion', n_out_channels=4, n_spatial_dims=1, n_groups=1, n_resblocks_per_group=4, n_channels=32, window_size=8, mask_smoothing=False, smooth_window_size=None, dilation_rates=[1], drop_rate=0.0, norm_mode='instance', mask_dropout=False, mask_drop_scales=[1, 5], mask_min_drop_rate=0.0, mask_max_drop_rate=0.5, mask_biasing=False, use_label_input=False) :
    ...
    mask_add = None
    if mask_biasing :
        mask_bias_concat = Concatenate(axis=-1)
        mask_add = Lambda(lambda x: x[0] + x[1])
    ...
    def _scrambler_func(example_input, mask_input=None, bias_input=None, label_input=None) :
        ...
        if mask_biasing :
            total_input = mask_bias_concat([total_input, bias_input])
        ...
        if mask_biasing :
            final_conv_out = mask_add([final_conv_out, bias_input])
...
def load_finetuning_model(batch_size, input_size_x, input_size_y, scrambler_mode='inclusion', n_out_channels=4, n_spatial_dims=1, mask_smoothing=False, smooth_window_size=None, norm_mode='instance', max_score_clip=4.) :
    ...
    bias_add = Lambda(lambda x: x[0] + x[1], name='ft_scrambler_bias_add')
    
    def _scrambler_func(sequence_input, drop_input, bias_input, pretrained_scores) :
        ...
        scores_out = bias_add([drop_multiply([clip_scores([mask_act_out, pretrained_scores]), drop_input]), bias_input])
...
def load_optimization_model(batch_size, input_size_x, input_size_y, scrambler_mode='inclusion', n_out_channels=4, n_spatial_dims=1, mask_smoothing=False, smooth_window_size=None, norm_mode='instance') :
    ...
    bias_add = Lambda(lambda x: x[0] + x[1], name='ot_scrambler_bias_add')
    
    def _scrambler_func(sequence_input, drop_input, bias_input) :
        ...
        scores_out = bias_add([drop_multiply([mask_act_out, drop_input]), bias_input])
...
class Scrambler :
    ...
    def __init__(self, n_inputs=1, n_classes=1, multi_input_mode='siamese', scrambler_mode='inclusion', input_size_x=1, input_size_y=100, n_out_channels=4, input_templates=None, input_backgrounds=None, batch_size=32, n_samples=32, sample_mode='st', gumbel_temp=0.5, zeropad_input=False, mask_dropout=False, mask_biasing=False, subset_size=None, model_prefix='', network_config={'n_groups' : 1, 'n_resblocks_per_group' : 4, 'n_channels' : 32, 'window_size' : 8, 'dilation_rates' : [1], 'drop_rate' : 0.25, 'norm_mode' : 'instance', 'mask_smoothing' : True, 'mask_smoothing_window_size' : 7, 'mask_smoothing_std' : 1.5, 'mask_drop_scales' : [1, 5], 'mask_min_drop_rate' : 0.0, 'mask_max_drop_rate' : 0.5, 'label_input' : False}) :
        ...
        self.mask_biasing = mask_biasing
        ...
        scrambler = load_scrambler_network(
            ...
            n_out_channels=self.n_out_channels * (1 if self.multi_input_mode != 'shared' else self.n_inputs),
        ...
        #Load sampler
        sampler = build_sampler(self.batch_size, self.input_size_x, self.input_size_y, n_classes=len(self.input_templates), n_samples=self.n_samples, subset_size=self.subset_size, sample_mode=self.sample_mode, gumbel_temp=self.gumbel_temp, n_channels=self.n_out_channels, model_prefix=self.model_prefix)
        ...
        for input_ix in range(self.n_inputs) :
            ...
            if self.mask_dropout and (self.multi_input_mode != 'shared' or input_ix == 0) :
            ...
            if self.mask_biasing and (self.multi_input_mode != 'shared' or input_ix == 0) :
                scrambler_biases.append(Input(shape=(self.input_size_x, self.input_size_y, 1), name='scrambler_bias_' + str(input_ix)))
            else :
                scrambler_biases.append(None)
        ...
        if self.multi_input_mode == 'siamese' or self.n_inputs == 1 :
            ...
            for input_ix in range(self.n_inputs) :
                ...
                scrambled_logit, importance_score = scrambler(scrambler_inputs[input_ix], scrambler_drops[input_ix], scrambler_biases[input_ix], scrambler_label)

                scrambled_logits.append(scrambled_logit)
                importance_scores.append(importance_score)
        elif self.multi_input_mode == 'joint' :
            ...
            scrambler_bias_concat = Lambda(lambda x: K.concatenate(x, axis=2))
            ...
            scrambler_bias = scrambler_bias_concat(scrambler_biases) if self.mask_biasing else None

            scrambled_logit, importance_score = scrambler(scrambler_input, scrambler_drop, scrambler_bias, scrambler_label)
            ...
            scrambler_input_concat = Lambda(lambda x: K.concatenate(x, axis=-1))
            
            scrambler_input = scrambler_input_concat(scrambler_inputs)
            scrambler_drop = scrambler_drops[0] if self.mask_dropout else None
            scrambler_bias = scrambler_biases[0] if self.mask_biasing else None

            scrambled_logit, importance_score = scrambler(scrambler_input, scrambler_drop, scrambler_bias, scrambler_label)

            scrambler_logit_split = Lambda(lambda x: [x[:, :, :, k*self.n_out_channels:(k+1)*self.n_out_channels] for k in range(self.n_inputs)])
            
            scrambled_logits = scrambler_logit_split(scrambled_logit)
            importance_scores = [importance_score for k in range(self.n_inputs)]

        for input_ix in range(self.n_inputs) :
                
            pwm_logit, pwm, sampled_pwm, _, sampled_mask = sampler(scrambler_classes[input_ix], scrambled_logits[input_ix], importance_scores[input_ix])
            
            if self.multi_input_mode == 'shared' and input_ix > 0 :
                
                var_mask = Lambda(lambda x: K.max(x[0] - x[1], axis=-1)[:, None, :, :, None])([scrambler_inputs[0], scrambler_inputs[input_ix]])
                sampled_pwm = Lambda(lambda x: x[0] * (1. - x[1]) + x[2][:, None, ...] * x[1])([sampled_pwms[0], var_mask, scrambler_inputs[input_ix]])
        ...
        scrambler_model = Model(
            scrambler_classes + scrambler_inputs + [scrambler_drop for scrambler_drop in scrambler_drops if scrambler_drop is not None] + [scrambler_bias for scrambler_bias in scrambler_biases if scrambler_bias is not None] + ([scrambler_label] if scrambler_label is not None else []),
            pwm_logits + pwms + sampled_pwms + importance_scores
        )

        #Initialize Templates and Masks
        initialize_templates(scrambler_model, self.input_templates, self.input_backgrounds_log, model_prefix=self.model_prefix)
    ...
    def save_model(self, model_path) :
        ...
        self.scrambler_model.save_weights(model_path)
        print('Saved scrambler model at %s ' % (model_path))

    def get_model(self) :
        return self.scrambler_model

    def interpret(self, x, y=None, drop=None, bias=None, group=None) :
        ...
        if bias is None :
            bias = [
                np.zeros((x[0].shape[0], x[0].shape[1], x[0].shape[2], 1)) for input_ix in range(self.n_inputs)
            ] if self.mask_biasing else []
        
        if not isinstance(bias, list) :
            bias = [bias]
        
        if len(drop) > 1 and self.multi_input_mode == 'shared' :
            drop = drop[:1]
        
        if len(bias) > 1 and self.multi_input_mode == 'shared' :
            bias = bias[:1]
        ...
        input_tensors = group + x + drop + bias + label
    ...
    def train(self, predictor, x_train, y_train, x_test, y_test, n_epochs, extra_input_train=None, extra_input_test=None, group_train=None, group_test=None, bias_train=None, bias_test=None, monitor_test_indices=None, monitor_batch_freq_dict={0 : 1, 1 : 5, 5 : 10}, adam_lr=0.0001, adam_beta_1=0.5, adam_beta_2=0.9, nll_mode='reconstruction', predictor_task='classification', custom_loss_func=None, reference='predictor', entropy_mode='target', entropy_bits=0., entropy_weight=1., entropy_mode_init=None, entropy_bits_init=0., entropy_weight_init=1., entropy_anneal_func=lambda val, epoch: 1.) :
        ...
        if bias_train is None :
            bias_train = [
                np.zeros((x_train[0].shape[0], x_train[0].shape[1], x_train[0].shape[2], 1)) for input_ix in range(self.n_inputs)
            ] if self.mask_biasing else []
            bias_test = [
                np.zeros((x_test[0].shape[0], x_test[0].shape[1], x_test[0].shape[2], 1)) for input_ix in range(self.n_inputs)
            ] if self.mask_biasing else []
        
        if not isinstance(bias_train, list) :
            bias_train = [bias_train]
            bias_test = [bias_test]
        
        if len(bias_train) > 1 and self.multi_input_mode == 'shared' :
            bias_train = bias_train[:1]
            bias_test = bias_test[:1]
        ...
        if n_trim_train > 0 :
            ...
            for i in range(len(bias_train)) :
                bias_train[i] = bias_train[i][:-n_trim_train]
        ...
        if n_trim_test > 0 :
            ...
            for i in range(len(bias_test)) :
                bias_test[i] = bias_test[i][:-n_trim_test]
        ...
        for input_ix in range(self.n_inputs) :
            ...
            if self.mask_biasing and (self.multi_input_mode != 'shared' or input_ix == 0) :
                scrambler_biases.append(Input(shape=(self.input_size_x, self.input_size_y, 1), name='t_scrambler_bias_' + str(input_ix)))
            else :
                scrambler_biases.append(None)
        ...
        if self.multi_input_mode == 'siamese' or self.n_inputs == 1 :
            ...
            for input_ix in range(self.n_inputs) :
                ...
                scrambled_logit, importance_score = self.scrambler(scrambler_inputs[input_ix], scrambler_drops[input_ix], scrambler_biases[input_ix], scrambler_label)
            ...
            scrambled_logit, importance_score = self.scrambler(scrambler_input, scrambler_drop, scrambler_bias, scrambler_label)
        ...
        for input_ix in range(self.n_inputs) :
            ...
            pwm_logit, pwm, sampled_pwm, pwm_mask, sampled_mask = self.sampler(scrambler_classes[input_ix], scrambled_logits[input_ix], importance_scores[input_ix])
        ...
        #Initial entropy cost
        entropy_loss_func_init = None
        if entropy_mode_init is not None and entropy_mode_init == 'target' :
            entropy_loss_func_init = get_target_entropy_sme_masked(x_start=0, x_end=self.input_size_x, y_start=0, y_end=self.input_size_y, target_bits=entropy_bits_init)
        elif entropy_mode_init is not None and entropy_mode_init == 'maximization' :
            entropy_loss_func_init = get_margin_entropy_ame_masked(x_start=0, x_end=self.input_size_x, y_start=0, y_end=self.input_size_y, max_bits=entropy_bits_init)
        ...
        entropy_losses = []
        if entropy_loss_func_init is None :
            for input_ix in range(self.n_inputs) :
            ...
            for input_ix in range(self.n_inputs) :
                entropy_loss = Lambda(lambda x: anneal_coeff * K.expand_dims(entropy_loss_func(x[0], x[1], x[2]), axis=-1) + (1. - anneal_coeff) * (entropy_weight_init / max(entropy_weight, 1e-7)) * K.expand_dims(entropy_loss_func_init(x[0], x[1], x[2]), axis=-1), name='entropy_' + str(input_ix))([
                    pwms[input_ix],
                    pwm_masks[input_ix],
                    x_means[input_ix]
                ])
        ...
        loss_model = Model(
            scrambler_classes + scrambler_inputs + [scrambler_drop for scrambler_drop in scrambler_drops if scrambler_drop is not None] + [scrambler_bias for scrambler_bias in scrambler_biases if scrambler_bias is not None] + ([scrambler_label] if scrambler_label is not None else []) + scrambler_extra_inputs,
            [nll_loss, entropy_loss]
        )

        #Initialize Templates and Masks
        initialize_templates(loss_model, self.input_templates, self.input_backgrounds_log, model_prefix=self.model_prefix)

        #Initialize Sequence Length Parameters
        initialize_backgrounds(loss_model, self.input_backgrounds)

        opt = keras.optimizers.Adam(lr=adam_lr, beta_1=adam_beta_1, beta_2=adam_beta_2) if self.loss_model_opt is None else self.loss_model_opt 
        ...
        if len(drop_train) > 1 and self.multi_input_mode == 'shared' :
            drop_train = drop_train[:1]
        ...
        if monitor_test_indices is not None :
            ...
            bias_track = [b[monitor_test_indices] for b in bias_test]
            label_track = [l[monitor_test_indices] for l in label_test]
            extra_input_track = [e[monitor_test_indices] for e in extra_input_test]
            
            monitor_loss_tensors = group_track + x_track + drop_track + bias_track + label_track + extra_input_track
            monitor_tensors = group_track + x_track + drop_track + bias_track + (label_track if self.label_input else [])
        ...
        if entropy_loss_func_init is not None :
            callbacks.append(EpochVariableCallback(anneal_coeff, entropy_anneal_func))
        
        train_history = loss_model.fit(
            group_train + x_train + drop_train + bias_train + label_train + extra_input_train,
            [dummy_train, dummy_train],
            shuffle=True,
            epochs=n_epochs,
            batch_size=self.batch_size,
            validation_data=(
                group_test + x_test + drop_test + bias_test + label_test + extra_input_test,
    ...
    def finetune(self, predictor, x, y, batch_size, n_iters, drop=None, bias=None, extra_input=None, group=None, pretrained_scores=None, norm_mode='instance', max_score_clip=4., adam_lr=0.01, adam_beta_1=0.5, adam_beta_2=0.9, nll_mode='reconstruction', predictor_task='classification', custom_loss_func=None, reference='predictor', entropy_mode='target', entropy_bits=0., entropy_weight=1., subset_size=None, cache_prefix_path=None, load_from_cache=False) :
        
        if pretrained_scores is None :
            #Collect pre-trained importance scores
            print("Generating pre-trained scores...")
            _, _, pretrained_scores = self.interpret(x, y=y, drop=drop, bias=bias, group=group)
        ...
        return self._optimize(predictor, finetuner, pretrained_scores, x, y, batch_size, n_iters, 'finetune', drop=drop, bias=bias, extra_input=extra_input, group=group, adam_lr=adam_lr, adam_beta_1=adam_beta_1, adam_beta_2=adam_beta_2, nll_mode=nll_mode, predictor_task=predictor_task, custom_loss_func=custom_loss_func, reference=reference, entropy_mode=entropy_mode, entropy_bits=entropy_bits, entropy_weight=entropy_weight, subset_size=subset_size, cache_prefix_path=cache_prefix_path, load_from_cache=load_from_cache)
    
    def optimize(self, predictor, x, y, batch_size, n_iters, drop=None, bias=None, extra_input=None, group=None, norm_mode='instance', adam_lr=0.01, adam_beta_1=0.5, adam_beta_2=0.9, nll_mode='reconstruction', predictor_task='classification', custom_loss_func=None, reference='predictor', entropy_mode='target', entropy_bits=0., entropy_weight=1., subset_size=None, cache_prefix_path=None, load_from_cache=False) :
        ...
        return self._optimize(predictor, optimizer, None, x, y, batch_size, n_iters, 'optimize', drop=drop, bias=bias, extra_input=extra_input, group=group, adam_lr=adam_lr, adam_beta_1=adam_beta_1, adam_beta_2=adam_beta_2, nll_mode=nll_mode, predictor_task=predictor_task, custom_loss_func=custom_loss_func, reference=reference, entropy_mode=entropy_mode, entropy_bits=entropy_bits, entropy_weight=entropy_weight, subset_size=subset_size, cache_prefix_path=cache_prefix_path, load_from_cache=load_from_cache)
    
    def _optimize(self, predictor, finetuner, pretrained_scores, x, y, batch_size, n_iters, opt_mode, drop=None, bias=None, extra_input=None, group=None, adam_lr=0.01, adam_beta_1=0.5, adam_beta_2=0.9, nll_mode='reconstruction', predictor_task='classification', custom_loss_func=None, reference='predictor', entropy_mode='target', entropy_bits=0., entropy_weight=1., subset_size=None, cache_prefix_path=None, load_from_cache=False) :
        ...
        if not isinstance(pretrained_scores, list) :
        ...
        if len(pretrained_scores) > 1 and self.multi_input_mode == 'shared' :
            pretrained_scores = pretrained_scores[:1]
        ...
        #Load sampler
        finetuning_sampler = build_sampler(batch_size, self.input_size_x, self.input_size_y, n_classes=len(self.input_templates), n_samples=self.n_samples, subset_size=subset_size, sample_mode=self.sample_mode, n_channels=self.n_out_channels, gumbel_temp=self.gumbel_temp, model_prefix='ft_')
        ...
        for input_ix in range(self.n_inputs) :
            ...
            if self.multi_input_mode != 'shared' or input_ix == 0 :
            ...
            if self.multi_input_mode != 'shared' or input_ix == 0 :
                scrambler_biases.append(Input(batch_shape=(batch_size, self.input_size_x, self.input_size_y, 1), name='f_scrambler_bias_' + str(input_ix)))
            if opt_mode == 'finetune' and (self.multi_input_mode != 'shared' or input_ix == 0) :
        ...
        if self.multi_input_mode == 'siamese' or self.n_inputs == 1 :
            ...
            for input_ix in range(self.n_inputs) :
                ...
                if opt_mode == 'finetune' :
                    scrambled_logit, importance_score = finetuner(scrambler_inputs[input_ix], scrambler_drops[input_ix], scrambler_biases[input_ix], scrambler_pretrained_scores[input_ix])
                else :
                    scrambled_logit, importance_score = finetuner(scrambler_inputs[input_ix], scrambler_drops[input_ix], scrambler_biases[input_ix])
            ...
            scrambler_input = scrambler_input_concat(scrambler_inputs)
            scrambler_drop = scrambler_drop_concat(scrambler_drops)
            scrambler_bias = scrambler_bias_concat(scrambler_biases)
            scrambler_pretrained_score = scrambler_pretrained_score_concat(scrambler_pretrained_scores) if opt_mode == 'finetune' else None
            
            scrambled_logit, importance_score = None, None
            if opt_mode == 'finetune' :
                scrambled_logit, importance_score = finetuner(scrambler_input, scrambler_drop, scrambler_bias, scrambler_pretrained_score)
            else :
                scrambled_logit, importance_score = finetuner(scrambler_input, scrambler_drop, scrambler_bias)
            ...
            scrambler_input = scrambler_input_concat(scrambler_inputs)
            scrambler_drop = scrambler_drops[0]
            scrambler_bias = scrambler_biases[0]
            scrambler_pretrained_score = scrambler_pretrained_scores[0] if opt_mode == 'finetune' else None
        ...
        for input_ix in range(self.n_inputs) :
            ...
            pwm_logit, pwm, sampled_pwm, _, sampled_mask = finetuning_sampler(scrambler_classes[input_ix], scrambled_logits[input_ix], importance_scores[input_ix])
        ...
        finetuning_model = Model(
            scrambler_classes + scrambler_inputs + scrambler_drops + scrambler_biases + scrambler_pretrained_scores,
            pwm_logits + pwms + sampled_pwms + importance_scores
        )
        ...
        for input_ix in range(self.n_inputs) :
            ...
            if self.multi_input_mode != 'shared' or input_ix == 0 :
                scrambler_biases.append(Input(batch_shape=(batch_size, self.input_size_x, self.input_size_y, 1), name='ft_scrambler_bias_' + str(input_ix)))
            if opt_mode == 'finetune' and (self.multi_input_mode != 'shared' or input_ix == 0) :
        ...
        for input_ix in range(self.n_inputs) :
            ...
            pwm_logit, pwm, sampled_pwm, pwm_mask, sampled_mask = finetuning_sampler(scrambler_classes[input_ix], scrambled_logits[input_ix], importance_scores[input_ix])
            ...
            #Define layer to deflate sample axis
            deflate_scrambled_sample = Lambda(lambda x: K.reshape(x, (batch_size * self.n_samples, self.input_size_x, self.input_size_y, self.n_out_channels)), name='t_deflate_scrambled_sample_' + str(input_ix))
        ...
        inflate_scrambled_prediction = Lambda(lambda x: K.reshape(x, (batch_size, self.n_samples, self.n_classes)), name='ft_inflate_scrambled_prediction')
        ...
        finetuning_loss_model = Model(
            scrambler_classes + scrambler_inputs + scrambler_drops + scrambler_biases + scrambler_pretrained_scores + ([scrambler_label] if scrambler_label is not None else []) + scrambler_extra_inputs,
            [nll_loss, entropy_loss]
        )
        ...
        input_tensors = group + x + drop + bias + pretrained_scores + label + extra_input
        input_tensors_pred = group + x + drop + bias + pretrained_scores + label
        ...
        if n_pad > 0 :
            ...
            input_tensors_pred = [
                np.concatenate([input_tensor, np.zeros(tuple([n_pad] + list(input_tensor.shape)[1:]))], axis=0)
                for input_tensor in input_tensors_pred
            ]
        ...
        for batch_ix in range(n_batches) :
            ...
            input_tensors_pred_batch = [
                input_tensor[batch_ix*batch_size:(batch_ix+1)*batch_size] for input_tensor in input_tensors_pred
            ]
            
            if load_from_cache and cache_prefix_path is not None :
                finetuning_model.load_weights(cache_prefix_path + "_batch_" + str(batch_ix) + ".h5", by_name=True)
                #finetuning_loss_model.load_weights(cache_prefix_path + "_batch_" + str(batch_ix) + "_loss.h5", by_name=True)
                
                #finetuning_loss_model._make_train_function()
                weight_values = None
                #with open(cache_prefix_path + "_batch_" + str(batch_ix) + "_opt.pkl", 'rb') as f:
            ...
            pred_bundle = finetuning_model.predict(x=input_tensors_pred_batch, batch_size=batch_size, verbose=False)
            ...
            if cache_prefix_path is not None :
                finetuning_model.save_weights(cache_prefix_path + "_batch_" + str(batch_ix) + ".h5")
                #finetuning_loss_model.save_weights(cache_prefix_path + "_batch_" + str(batch_ix) + "_loss.h5")
                
                #symbolic_weights = getattr(opt, 'weights')
                #weight_values = K.batch_get_value(symbolic_weights)
                #with open(cache_prefix_path + "_batch_" + str(batch_ix) + "_opt.pkl", 'wb') as f:
