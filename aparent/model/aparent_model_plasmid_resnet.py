...
def load_residual_network(batch_size=32, n_groups=1, n_resblocks_per_group=4, n_channels=32, window_size=8, dilation_rates=[1], drop_rate=0.0) :
    ...
    #extend_tensor = Lambda(lambda x, batch_size=batch_size: K.concatenate([x, K.zeros((batch_size, 1, 1, 1))], axis=2), name='aparent_extend_tensor')
    ...
    lib_conv = LocallyConnected2D(1, (1, 1), strides=(1, 1), padding='valid', activation='linear', kernel_initializer='zeros', name='aparent_lib_conv')
